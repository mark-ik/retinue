...
use retinue::link::{
    LinkMode, LinkTrailer, PendingLink, CTX_RESOURCE, CTX_RESOURCE_ADV, CTX_RESOURCE_HMU,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data: Vec<u8> = (0..120_000u32).map(|i| (i.wrapping_mul(2_654_435_761) >> 8) as u8).collect();
    ...
    send(&mut iface, &link.sealed_packet(CTX_RESOURCE_ADV, &out.advertisement().pack(), &iv(3))).await;
    println!("SENT_ADV");
        ...
        match packet.context {
            0x03 => {
                ...
                if req.exhausted && let Some(last) = req.last_map_hash {
                    ...
                    send(&mut iface, &link.sealed_packet(CTX_RESOURCE_HMU, &hmu, &iv(ivc))).await;
                    println!("SENT_HMU");
                }
            }
            0x05 => {
