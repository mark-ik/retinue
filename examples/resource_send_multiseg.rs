...
use retinue::link::{
    Link, LinkMode, LinkTrailer, PendingLink, CTX_RESOURCE, CTX_RESOURCE_ADV, CTX_RESOURCE_HMU,
};
use retinue::packet::{Packet, PacketType};
use retinue::resource::{self, Outgoing, MAX_SEGMENT_SIZE};
...
async fn send_segment(
    ...
    *ivc += 1;
    send(iface, &link.sealed_packet(CTX_RESOURCE_ADV, &out.advertisement().pack(), &iv(*ivc))).await;
    ...
    while tokio::time::Instant::now() < deadline {
        ...
        match packet.context {
            0x03 => {
                ...
                if req.exhausted && let Some(last) = req.last_map_hash {
                    *ivc += 1;
                    let hmu = out.hmu_after(&last);
                    send(iface, &link.sealed_packet(CTX_RESOURCE_HMU, &hmu, &iv(*ivc))).await;
                }
            }
            0x05 => {
                return Ok(matches!(resource::parse_proof(&packet.payload), Some((_h, p)) if p == expected));
            }
            _ => {}
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let data: Vec<u8> = (0..TOTAL as u32).map(|i| (i.wrapping_mul(2_654_435_761) >> 8) as u8).collect();
    ...
    let original_hash = resource::resource_hash(&data[..data.len().min(MAX_SEGMENT_SIZE)], &seg0_rh);
    ...
    for (idx, chunk) in data.chunks(MAX_SEGMENT_SIZE).enumerate() {
        ...
        let out = Outgoing::new(chunk, &token, rh, false)
            .with_segment(idx as i64 + 1, segments as i64, data.len() as u64, original_hash);
